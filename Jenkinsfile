pipeline {
  ...
  environment {
    REACT_APP_SERVICE_URL = credentials(evaluate(env.BRANCH_NAME + '_REACT_APP_SERVICE_URL'))
    databaseUrl = credentials(evaluate(env.BRANCH_NAME + '_databaseUrl'))
    databaseUsername = credentials(evaluate(env.BRANCH_NAME + '_databaseUsername'))
    databasePassword = credentials(evaluate(env.BRANCH_NAME + '_databasePassword'))
    jwtSecretKey = credentials(evaluate(env.BRANCH_NAME + '_jwtSecretKey'))
    dockerhubUsername = credentials(evaluate(env.BRANCH_NAME + '_dockerhubUsername'))
    dockerhubPassword = credentials(evaluate(env.BRANCH_NAME + '_dockerhubPassword'))
